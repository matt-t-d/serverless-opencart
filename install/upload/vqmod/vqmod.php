...
abstract class VQMod {
	...
	public static $readOnlySystem = false;						// In a read only system, assume 'cached' files are correct
     ...
     * @param bool $readOnlySystem
     * @return null
     * @description Startup of VQMod
     */
	public static function bootup($path = false, $logging = true, $readOnlySystem = false) {
		...
		self::$readOnlySystem = $readOnlySystem;
        if (self::$readOnlySystem) {
            $logging = false;
        }
	...
	private static function _getMods() {
		...
		} elseif(file_exists($modCache) && (self::$readOnlySystem || filemtime($modCache) >= self::$_lastModifiedTime)) {
